...
class TradingAgent:
    ...
    def _decide(self, context, assets, has_active_trades=False):
        ...
        system_prompt = (
            ...
            "Per-asset performance memory (P3.3): each market_data entry now includes recent_performance — win rate, average win/loss, net PnL, and last-5 outcomes over the last 20 closed trades for that asset. A null value means fewer than 2 records exist (insufficient data — trade normally). With 5 or fewer closed trades, recent_performance is NOT a reason to withhold a trade — the sample is too small to be meaningful. When recent_performance shows a persistent losing pattern (>5 trades, win_rate < 30%, net_pnl negative), require EXCEPTIONAL conviction before re-entering. Default to hold unless the current setup is qualitatively different from the prior failures — note in your reasoning how this setup differs.\n\n"
            f"Volume context (per asset): each market_data entry now includes vol_spike_ratio = current bar volume / 20-bar SMA volume on both 5m and 4h. Low-conviction volume (5m vol_spike_ratio < {float(CONFIG.get('min_vol_spike_ratio') or 0.3):.1f}) may reduce allocated size; the risk manager will apply a soft size reduction on weak tape. A 5m spike > 2.0 with aligned direction is a real-time momentum confirmation.\n\n"
            ...
            "- Each item inside trade_decisions must contain the keys: asset, action, allocation_usd, order_type, limit_price, tp_price, sl_price, exit_plan, exit_rules, rationale, conviction.\n"
            ...
            "- conviction field: include a \"conviction\" key (float 0.0–1.0) in each trade_decisions item expressing your confidence level. 1.0 = highest conviction, 0.1 = minimal probe. The system multiplies your allocation_usd by clamp(conviction, 0.1, 1.0) before applying position caps. If you set high conviction on a bad trade, the loss is proportionally larger — be accurate.\n"
        )
